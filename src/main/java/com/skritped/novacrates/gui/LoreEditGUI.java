...
public class LoreEditGUI extends InventoryGUI {
    ...
    public LoreEditGUI(JavaPlugin plugin, GUIManager guiManager, CrateManager manager,
        ...
        if (r != null && r.getLore() != null) {
            lines.addAll(r.getLore());
        }
    }
    ...
    private void reopen(Player player) {
        Bukkit.getScheduler().runTask(plugin, () ->
                guiManager.openGUI(new LoreEditGUI(plugin, guiManager, manager, crateId, rewardId), player));
    }

    @Override
    protected Inventory createInventory() {
        return Bukkit.createInventory(null, 54, Text.legacy("&8Lore &7» &f" + rewardId));
    }
    ...
    public void decorate(Player player) {
        ...
        getInventory().clear();
        int slot = 0;
        for (int i = 0; i < lines.size() && slot < 45; i++) {
            final int idx = i;
            ...
            addButton(slot++, new InventoryButton().creator(p -> {
                ...
                if (m != null) {
                    ...
                    m.setLore(List.of(
                            Text.legacy(line.isEmpty() ? "&8(pusta)" : line),
                ...
                if (e.isRightClick()) {
                    ...
                    ((com.skritped.novacrates.NovaCratesPlugin) plugin).getAnvilInput().open(
                            player, "Linia lore (użyj &)", line, false,
                            text -> {
                                if (idx < lines.size()) lines.set(idx, text);
                                save();
                                reopen(player);
                            },
                            () -> reopen(player)
        ...
        addButton(45, new InventoryButton().creator(p -> named(Material.EMERALD, "&a+ Dodaj linię")).consumer(e -> {
            player.closeInventory();
            ((com.skritped.novacrates.NovaCratesPlugin) plugin).getAnvilInput().open(
                    player, "Nowa linia lore", "&7Tekst...", false,
                    text -> {
                        lines.add(text);
                        save();
                        reopen(player);
    ...
    private void save() {
        manager.updateRewardLore(crateId, rewardId, new ArrayList<>(lines));
    }
